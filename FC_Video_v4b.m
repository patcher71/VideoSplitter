classdef FC_Video_v4b < matlab.apps.AppBase

    % Properties that correspond to app components
    properties (Access = public)
        VideoSplitterUIFigure          matlab.ui.Figure
        VideoDurationsEditField        matlab.ui.control.NumericEditField
        VideoDurationsEditFieldLabel   matlab.ui.control.Label
        TruncateVideosCheckBox         matlab.ui.control.CheckBox
    ...
    if app.IndividualTrialsButton.Value == 1
        ...
        for i = 1:totalSegments
            ...
            open(outputVideo);
             ...
             if app.TruncateVideosCheckBox.Value == 1
    % User wants truncated videos
    truncateDuration = app.VideoDurationsEditField.Value; % in seconds
    framesToKeep = round(truncateDuration * frameRate);
    actualEndFrame = min(segmentInfo.StartFrame(i) + framesToKeep - 1, segmentInfo.EndFrame(i));
else
    % Use full segment
    actualEndFrame = segmentInfo.EndFrame(i);
end

% Write frames from start to actualEndFrame
for j = segmentInfo.StartFrame(i):actualEndFrame
    frame = read(v, j);
   ...
   % Initialize progress dialog
    ...
    for i = 1:totalSegments
        ...
        % Determine end frame based on truncation setting
        if app.TruncateVideosCheckBox.Value == 1
            % User wants truncated videos
            truncateDuration = app.VideoDurationsEditField.Value; % in seconds
            framesToKeep = round(truncateDuration * frameRate);
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.CreateVideoSegmentsButton.Position = [617 488 148 22];
            ...
            % Create TruncateVideosCheckBox
            app.TruncateVideosCheckBox = uicheckbox(app.VideoSplitterUIFigure);
            app.TruncateVideosCheckBox.Text = 'Truncate Videos ';
            app.TruncateVideosCheckBox.Position = [784 488 112 22];

            % Create VideoDurationsEditFieldLabel
            app.VideoDurationsEditFieldLabel = uilabel(app.VideoSplitterUIFigure);
            app.VideoDurationsEditFieldLabel.HorizontalAlignment = 'right';
            app.VideoDurationsEditFieldLabel.Position = [906 488 102 22];
            app.VideoDurationsEditFieldLabel.Text = 'Video Duration (s)';

            % Create VideoDurationsEditField
            app.VideoDurationsEditField = uieditfield(app.VideoSplitterUIFigure, 'numeric');
            app.VideoDurationsEditField.Position = [1023 488 100 22];
